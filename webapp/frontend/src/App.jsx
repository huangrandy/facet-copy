...
import SuggestionSection from './components/suggestion/SuggestionSection.jsx';
...
function App() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const newFeatures = Object.entries(formatDict.feature_names).map(([key, value], index) => {
                ...
                const current_value = selectedInstance[key]
                const semantic_min = formatDict.semantic_min[value] ?? 0;
                const semantic_max =
                    formatDict.semantic_max[value] ?? (
                        current_value === 0
                            ? default_max
                            : current_value * 2
                    )
                console.log(value, selectedInstance[key], semantic_min, semantic_max)
                const lower_constraint = semantic_max * 0.25
                const upper_constraint = semantic_max * 0.75
    ...
    if (isLoading) {
        ...
        return (
            <div className="super-div" style={{ display: "flex" }}>
                <div id="sidebar">
                    <div className="header">
                        <h1 id="app-logo">FACET</h1>
                        <h4 style={{ margin: "10px 0px", color: "gray", fontSize: 18 }}>Loan Application (Demo)</h4>
                    </div>
                ...
                <div className='card-grid'>
                    <div id="back">
                        <button className="back-welcome-button" onClick={backToWelcomeScreen}>
                            <img className="grayscale" src={backArrowIcon} alt="Back Arrow Icon" />
                            Back to {formatDict["scenario_terms"]["instance_name"]} Selection
                        </button>
                    </div>
